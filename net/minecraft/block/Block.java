...
public class Block
{
    ...
    public void addCollisionBoxesToList(final World worldIn, final BlockPos pos, final IBlockState state, final AxisAlignedBB mask, final List<AxisAlignedBB> list, final Entity collidingEntity) {
        final BlockCollisionEvent event = new BlockCollisionEvent(this.getCollisionBoundingBox(worldIn, pos, state), state.getBlock(), pos.getX(), pos.getY(), pos.getZ());
        Client.getInstance().getEventBus().post((Object)event);
        if (event.isCancelled()) {
            return;
        }
        final AxisAlignedBB axisalignedbb = event.getAxisAlignedBB();
