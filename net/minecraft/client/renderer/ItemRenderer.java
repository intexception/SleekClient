...
public class ItemRenderer
{
    ...
    public void renderItemInFirstPerson(final float partialTicks) {
        ...
        if (this.itemToRender != null) {
            final boolean fakeAB = Client.getInstance().getModuleManager().getModuleByName("Animations").isToggled() && KillAura.isBlocking && Client.getInstance().getModuleManager().getModuleByName("KillAura").isToggled();
            if (this.itemToRender.getItem() instanceof ItemMap) {
                this.renderItemMap(entityplayersp, f3, f, f2);
            }
            else if (entityplayersp.getItemInUseCount() > 0 || fakeAB) {
                final EnumAction enumaction = fakeAB ? EnumAction.BLOCK : this.itemToRender.getItemUseAction();
                switch (enumaction) {
                    case NONE: {
                    ...
                    case BLOCK: {
                        if (Client.getInstance().getModuleManager().getModuleByName("Animations").isToggled()) {
                            final Animations anim = (Animations)Client.getInstance().getModuleManager().getModuleByName("Animations");
